...
from .socialmedia import BASILICA
...
def predict_user(user1_name, user2_name, tweet_text, cache=None):
    ...
    if cache and cache.exists(user_set):
        ...
        user1_embeddings = np.array([tweet.embedding for tweet in user1.messages])
        user2_embeddings = np.array([tweet.embedding for tweet in user2.messages])
        #split into array
        embeddings = np.vstack([user1_embeddings, user2_embeddings])
        labels = np.concatenate([np.ones(len(user1.messages)),
                                 np.zeros(len(user2.messages))])
