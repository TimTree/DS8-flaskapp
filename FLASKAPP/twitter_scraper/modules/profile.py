...
class Profile:
    ...
    def __parse_profile(self, page):
        ...
        try:
            self.location = page.find(attrs={"class":"ProfileHeaderCard-locationText u-dir"}).contents[0].strip()
        ...
        # parse user id
        self.userid = page.find(attrs={"class":"ProfileNav"}).attrs['data-user-id']
    ...
    def __process_paragraph(self, contents):
        ...
        for i in contents:
            try:
                ...
                if i.name=="a":
                    tmp_txt, tmp_lnk = __process_paragraph(i.contents)
                    links+=tmp_lnk
